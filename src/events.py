    ...
    def apply_event(self, player):
        ...
        for key, value in self.effect.items():
            ...
            if key == "damage" or key == "huge_damage":
                ...
                while True:
                    choice = input("would you like to give item to the stranger? y/n: ").strip().lower()
                    if choice != "y":
                        break
                    else:
                        exist_items = []
                        ...
                        if len(exist_items) == 0:
                            ...
                            display_success_message(f"You have traded 1 item with stranger ({random_item}): -1")
